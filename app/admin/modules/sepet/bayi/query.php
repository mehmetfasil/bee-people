...
switch (ACT){
 ...
 case "save":
  ...
  if (strlen(getvalue("isim")) > 0) {
   if (getvalue("id",0) > 0) {
    //Update
    $query = "UPDATE `sepet_bayiler` SET "
    ...
    //Insert
    $query = "INSERT INTO `sepet_bayiler` VALUES ("
    . "null, "
 ...
 case "values":
  ...
  $query = "SELECT * FROM `sepet_bayiler` WHERE `id`=".getvalue("id",0);
  $select = new query($query, CONN);
 ...
 case "delete":
  ...
  $query = "SELECT `logo` FROM `sepet_bayiler` WHERE `id`=".getvalue("id",0);
  $select = new query($query, CONN);
  ...
  //Kaydı silelim
  $query = "DELETE FROM `sepet_bayiler` WHERE `id`=".getvalue("id",0);
