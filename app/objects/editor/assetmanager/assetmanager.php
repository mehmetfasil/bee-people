...
if(isset($_FILES["File1"]))
  ...
  if($MaxFileSize && ($_FILES['File1']['size'] > $MaxFileSize))
    {
    $sMsg = "ï¿½zin Verilen Maximum Dosya Boyutunu Aï¿½tï¿½nï¿½z";
    }
...
Function writeFolderSelections()
  ...
  echo "<select style='display:none;' name='selCurrFolder' id='selCurrFolder' class='inpSel'>";
    ...
    if($sItem=="."||$sItem=="..")
      ...
      if(!$fIsDirectory)
        ...
        if($bDisplay)
          ...
          // silmeyi kapatï¿½yoruz // onclick function deleteFile(".$nIndex.")
         /* echo "<u style='font-size:10px;cursor:pointer;color:crimson' ".$sFolderAdmin.">";
...
function downloadFile(index)
  ...
  window.open(pathPrefix + sFile_RelativePath)
  }
function selectFile(index)
  ...
  if(sFile_Extension.toUpperCase()=="GIF" || sFile_Extension.toUpperCase()=="JPG" || sFile_Extension.toUpperCase()=="PNG")
    {
    sHTML = "<img src=\"" + pathPrefix + sFile_RelativePath + "\" >"
    }
  //SWF
    ...
    sHTML = "<object "+
      ...
      " <param name=movie value='" + pathPrefix + sFile_RelativePath+"'>" +
      " <param name=quality value='high'>" +
      " <embed src='" + pathPrefix+sFile_RelativePath+"' " +
      "   width='100%' " +
      "   height='100%' " +
      "   quality='high' " +
  ...
  //Video
    ...
    {
    sHTML = "<embed src='" + pathPrefix+sFile_RelativePath+"' hidden=false autostart='true' type='video/avi' loop='true'></embed>";
    }
  //Sound
  else if(sFile_Extension.toUpperCase()=="WMA"||sFile_Extension.toUpperCase()=="WAV"||sFile_Extension.toUpperCase()=="MID")
    {
    sHTML = "<embed src='" + pathPrefix+sFile_RelativePath+"' hidden=false autostart='true' type='audio/wav' loop='true'></embed>";
    }
  //Files (Hyperlinks)
  else
    {
...
/*function deleteFile(index)
  ...
  if (confirm(getTxt("Delete this file ?")) == true)
    {
    sFile_RelativePath = pathPrefix + document.getElementById("inpFile"+index).value;
